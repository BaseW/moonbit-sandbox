...
WebAssembly.instantiateStreaming(fetch(new URL(wasmPath, import.meta.url)), importObject)
    .catch(async (reason) => {
        if (reason instanceof TypeError && reason.cause.message === "not implemented... yet...") {
            /// Node.js
            const { readFile } = await import('node:fs/promises');
            await WebAssembly.instantiate(await readFile(new URL(wasmPath, import.meta.url)), importObject);
        } else {
            throw reason;
        }
    })
